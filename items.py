...
def add_item(game_name, game_username, availability_time,
             availability_start, availability_end, other_info, user_id, classes):
    sql = """INSERT INTO items (game_name, game_username, availability_time,
             availability_start, availability_end, other_info, user_id)
             VALUES (?, ?, ?, ?, ?, ?, ?)"""
    db.execute(sql, [game_name, game_username, availability_time,
                     availability_start, availability_end, other_info, user_id])
...
def get_items():
    sql = """SELECT items.id, items.game_name,
             items.availability_time, users.id user_id, users.username
             FROM items, users
...
def get_item(item_id):
    sql = """SELECT items.id, items.game_name, items.game_username,
             items.availability_time, items.availability_start, items.availability_end,
             items.other_info, users.id user_id, users.username
             FROM items, users
             WHERE items.user_id = users.id
             AND items.id = ?"""
...
def update_item(item_id, game_name, game_username, availability_time,
                availability_start, availability_end, other_info, classes):
    sql = """UPDATE items SET game_name = ?,
                          ...
                          WHERE id = ?"""
    db.execute(sql, [game_name, game_username, availability_time,
                     availability_start, availability_end, other_info, item_id])
...
def find_items(query):
    sql = """SELECT DISTINCT items.id, items.game_name, items.availability_time,
